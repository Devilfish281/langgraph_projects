# src/langgraph_projects/level-2/chatbot_external_memory6.py
"""
...
import sqlite3  # added Line
from pathlib import Path  # added Line

DB_REL_PATH = "state_db/example.db"  # added Line
DB_PATH = (Path(__file__).resolve().parent / DB_REL_PATH).resolve()  # added Line
DB_EXISTS_BEFORE = DB_PATH.exists()  # added Line


def _open_sqlite(db_path: Path) -> sqlite3.Connection:  # Changed Code
    db_path.parent.mkdir(parents=True, exist_ok=True)  # added Line
    logger.info("Using sqlite DB at: %s", db_path)  # added Line
    return sqlite3.connect(str(db_path), check_same_thread=False)  # Changed Code


def chatbot_with_sqlite_memory():
    """
    ## Sqlite
...
def build_app():
    ...
    graph = chatbot_with_sqlite_memory()
...
if __name__ == "__main__":
    ...
    pass_counter = 1 if DB_EXISTS_BEFORE else 0

    if pass_counter == 0:
        logger.info("DB did not exist yet -> running pass 0")
        ##############################################
        ...
        display_graph_if_enabled(
            ...
            output_name="chatbot_with_sqlite_memory.png",
        )
        ...
        logger.info(graph_state)

        logger.info("Program Done pass 0.")

    if pass_counter == 1:
        logger.info("DB already exists -> running pass 1")
        """
        ### Persisting state
        ...
        logger.info("Program Done pass 1.")
