...
from langgraph.checkpoint.postgres import PostgresSaver  # Added Code
from langgraph.graph import END, START, MessagesState, StateGraph
from langgraph.graph.message import add_messages
from langgraph.prebuilt import ToolNode, tools_condition
from langgraph.store.base import BaseStore
from langgraph.store.memory import InMemoryStore
from langgraph.store.postgres import PostgresStore  # Added Code
...
_PG_CHECKPOINTER_CM = None  # Added Code
_PG_CHECKPOINTER = None  # Added Code
_PG_STORE_CM = None  # Added Code
_PG_STORE = None  # Added Code
_PG_LOCK = threading.Lock()  # Added Code
...
def init_postgres():
    global _PG_CHECKPOINTER_CM, _PG_CHECKPOINTER, _PG_STORE_CM, _PG_STORE

    if _PG_CHECKPOINTER is not None and _PG_STORE is not None:
    ...
    with _PG_LOCK:
        ...
        # DB_URI must be set in docker compose env

        _PG_CHECKPOINTER_CM = PostgresSaver.from_conn_string(db_uri)
        _PG_CHECKPOINTER = _PG_CHECKPOINTER_CM.__enter__()
        _PG_CHECKPOINTER.setup()

        _PG_STORE_CM = PostgresStore.from_conn_string(db_uri)
...
if __name__ == "__main__":
    ...
    if use_postgres:
        ...
        exit(1)  # Exit the application with an error code to indicate the issue
